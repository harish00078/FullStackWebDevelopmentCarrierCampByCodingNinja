...
// here we are importing the (StyleRoot) component from the (radium):
// through which we are able to use the (functions) and (sudo-classes) of the (css)under our styling object:and we will gave those styling objects to our application components:
...
function App() {
  ...
  return (
    ...
    // here we cover all the routes of our application:under the (StyleRoot) component of (radium) library:so that we can use the (css) sudo-classes:In any of our application components:

    // but this (styleRoot) components does is:it will rap our hole application under the brand new-div which is followed by a (root) style sheet:In simple words its basically adding the new-css-styles to those roots or roots-sheets of our application in there runtime:
